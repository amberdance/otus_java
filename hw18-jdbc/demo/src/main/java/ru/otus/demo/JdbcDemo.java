...
public class JdbcDemo {
    ...
    private void selectRecord(Connection connection, int id) throws SQLException {
        try (var pst = connection.prepareStatement("select * from test")) {

            try (var rs = pst.executeQuery()) {
                while (rs.next()) {
                    logger.info("id:{}: value:{}", rs.getInt("id"), rs.getString("name"));
                }
            }
        }
    }

    private void deleteRecords(Connection connection) throws SQLException {
        try (var pst = connection.prepareStatement("delete from test")) {
            pst.execute();
        }
    }
}
